...
//数据库查询操作
function find_in_db(data_region_search_key, time, time_seach_key, filter) {
    ...
    var promise = new Promise(function (resolve, reject) {
        ...
        if (time_seach_key == '月') {
            Comment.aggregate([
                {
                    $match: {
                        "data_region_search_key": data_region_search_key, 'comment_month': time,
                        "data_source": '景点'
                    }
                },
            ...
            Comment.aggregate([
                {
                    $match: {
                        "data_region_search_key": data_region_search_key, 'comment_week': time,
                        "data_source": '景点'
                    }
                },
                {$group: {_id: "$data_website", "value": filter['value']}},
...
router.post('/', function (req, res, next) {
    ...
    var cur_spot = req.body.cur_spot;
